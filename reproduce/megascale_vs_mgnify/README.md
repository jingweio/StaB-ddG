...
- ✅ **作为可泛化的 absolute-ΔG / ΔΔG 训练数据集**：MGnify 在序列与结构广度上比 Megascale 大 **~1000×**。〔**订正 2026-08-16**：此处原写“并**首次**大规模测定绝对折叠自由能 ΔG”——**错**。Megascale(Tsuboyama 2023)**本来就有 absolute ΔG**(CSV 列 `deltaG`/`deltaG_t`/`deltaG_c`/`dG_ML`，以及相对量 `ddG_ML`)，且**两者用的是同一套 cDNA display proteolysis 方法、同一实验室**(Tsuboyama & Rocklin 均为两篇作者)，故 schema 几乎一致、ΔG 都落在 ~[-1,5] kcal/mol(这也是两者同属“cDNA 数据集 → sigmoid scaling ON”的原因)。论文自己的定位是:Megascale 的 ΔG 数据 “have also enabled new progress at predicting ΔG”，但 “the limited structural diversity of the Megascale dataset is insufficient… and likely too sparse to train accurate, generalizable predictors of ΔG”。→ **MGnify 的真正差异化是多样性/广度(211,020 vs 199 结构簇)，不是“首次测 ΔG”。**〕论文证据显示：单独用 MGnify 训练 > 单独用 Megascale 训练（ESM3 LoRA：**0.87 vs 0.73**），且 MGnify-trained 模型在 **Megascale 自己的点突变 benchmark** 上追平了用 Megascale 训练的 ThermoMPNN → **功能上可取代 Megascale**。
...
### (b) 突变扫描深度 —— 定义、差异，以及为什么 Megascale 在这方面更强

#### b.1 如何定义”突变扫描深度”

对一个长度为 *L* 的野生型结构域，它的**单点突变邻域**大小是 **19·L**（每个位点可换成另外 19 种氨基酸）。”深度”有两个可量化侧面：

1. **Site-saturation 完整度** = 实测到的相异单点突变数 ÷ (19·L)。=100% 即”把这个蛋白所有可能的单点突变都测了一遍”。
2. **每个域被实测的变体数**（含突变体 / indel）。

> 深度高 ⇒ 能**完整刻画”单个蛋白”的突变-稳定性地形图**（适合 ΔΔG、局部突变效应、epistasis、位点特异分析）。深度低、广度高 ⇒ 刻画的是**”蛋白宇宙”里不同蛋白之间**的绝对稳定性差异。
>
> ⚠️ **分母是 19·L（整个域），不是 19（单个位点）。** 一个 71-aa 域的单点突变空间是 19×71≈**1,349** 个。所以”平均每域测 ~1 个替换”对应 1/1,349≈**0.07%**，而不是 1/19=5.3%（后者是”某*一个*位点测了 1/19 种氨基酸”的位点级饱和度）。更关键：MGnify **67.9% 的域连一个替换都没有**（只有 WT），所以中位 site-saturation 其实是 **0%**，均值才被少数有突变的域拉到 ~0.07%。

#### b.2 两个数据集在深度上的真实差异（实算）

![mutation scanning depth](figures/fig3_mutation_depth.png)

| 深度指标 | Megascale | MGnify | 来源 |
|---|---:|---:|---|
| 每个 WT 域的变体数（中位 / 均值）| **1,324 / 1,616**（min 404, max 9,032）| **1 / 1.81**（max 4）| [数据] |
| 单点替换 site-saturation（均值）| **99.1%**（中位 99.9%）| **≈0.07%**（≈1 替换 ÷ 19·L≈1,349）| [数据] |
| 每个域被突变的位点比例 | **100%**（每个位点都测）| 绝大多数域 0 个突变位点 | [数据] |
| 每个位点平均测得的替换数 | **18.87 / 19** | ≪1 | [数据] |
| 仅有 WT、无任何突变体的域占比 | ~0% | **67.9%**（532,988 个域中 361,635 个）| [数据] |

**MGnify 不是”均匀地浅”——它的深度被拆成了三块**（这是上次粗略表述的精确化）：

| 子类 | 实算结果 | 性质 |
|---|---|---|
| **替换 substitution** | 532,988 个域里 **67.9% 只有 WT**；有突变体的仅 32.1%（171,353 个），且最多 3 个（直方图 1→361,635 / 2→6,238 / 3→68,715 / 4→96,400）| **浅而广**，site-sat ≈ 0 |
| **插入/删除 indel** | **只有 6,000 个域**被加 indel，但每个 **~49–50 条**（min 44 / max 50，共 295,607 条）| **深而专**：仅覆盖 0.9% 的域，却近饱和 |
| **野生型 WT** | 704,424 个不同天然域，各测一个绝对 ΔG | 预算几乎全花在广度上 |

> 所以严格说：在**替换突变扫描深度**上 Megascale 压倒性领先；在 **indel 扫描深度**上 MGnify 在它选的 6,000 个域上反而做得很深（这正是它能当 indel benchmark 的原因）。但 indel 之外，MGnify 没有任何一个域接近 Megascale 那种全位点饱和。

#### b.3 为什么”深度”是 Megascale 的优势（6 个理由）

1. **ΔΔG 本质是”配对量”，必须 WT 与突变体在同一域里都测到。** ΔΔG = ΔG(mut) − ΔG(WT)；要得到一个*实测*的 ΔΔG，必须在**同一条骨架**上同时有 WT 和该突变体。Megascale 对 479 个蛋白几乎每个可能单点突变都有这对数据；MGnify 平均每蛋白只有 ~1 个突变体、68% 一个都没有。问”突变 X 对蛋白 P 的影响是多少”，Megascale 几乎总能给真值，MGnify 几乎从不能。
2. **同蛋白内的受控对照分离出干净的突变信号。** 深扫是**受控实验**：同骨架、同序列环境、同批次，只改一个残基，WT 内参抵消了”每个蛋白各自的系统误差/标定偏移/表达差异”。靠”不同蛋白之间比 ΔG”是**观察性**的，混杂了蛋白与蛋白间的一切差异。深度 → 因果对照；广度 → 嘈杂横截面。
3. **完整 site-saturation = 一张完整的”每残基稳定性地形图”。** 有了”每位点 × 每种氨基酸”，才能定位稳定性热点（埋藏核心 vs 表面）、看每位点完整效应分布、找稳定化突变、量化位点容忍度。从”1 个突变/蛋白”做不出这些——这是**整张地图**与**单个像素**之别。
4. **上位效应（epistasis / 非加性）需要同一蛋白里的多个突变。** Megascale 有 210,118 个双突变（同域成对），可检验 ΔΔG(A)+ΔΔG(B) 是否 = ΔΔG(A+B)；MGnify 的 15 万双突变摊到几十万域上，做不了同域分析。
5. **它是 ΔΔG 模型的天然监督，也是评测的黄金标准——一个很说明问题的事实：** ThermoMPNN、StaB-ddG 预测的就是局部突变效应，需要稠密的单蛋白突变数据；而要*评测*”模型能否把一个蛋白的所有突变排对序”，必须有该蛋白 ~所有突变的真值——只有饱和 DMS 提供得了。**正因如此，MGnify 这篇论文做 ΔΔG benchmark 时仍回头用 Megascale 的 28,172 个点突变集**（Fig 3b）——它自家数据每蛋白太浅，担不起这个角色。这是 Megascale 深度优势最直接的证据。
6. **单蛋白的统计功效。** 每骨架 ~1,600 个突变，可平均掉该域噪声、拟合位点级模型、检出微小效应；1 个测量做不到任何这些。

#### b.4 为什么 MGnify 没做到同样深度——是预算分配的权衡，不是缺陷

同一套 cDNA display proteolysis、同样有限的寡核苷酸合成能力（每库 ~57–90 万条）。**总测量预算固定时只能二选一**：

- 少数骨架 × 深扫：Megascale = 776,298 ÷ 479 ≈ **1,620 条/域**；
- 海量骨架 × 浅测：MGnify 把 186 万条里的绝大部分用于”给 70 万个不同天然域各测一个绝对 ΔG”。

MGnify 的科学目标是覆盖 211,020 个序列簇的**多样性**以训练**可泛化的绝对 ΔG 预测器**，所以预算几乎全投给广度；它在物理上无法同时把每个域也饱和扫描。这是**有意的设计选择**，不是数据质量缺陷。

> **一句话**：深度衡量的是”能否完整刻画单个蛋白的突变-稳定性地形”。Megascale 对 479 个蛋白把这张图画满了（~99%），所以在 ΔΔG / 局部突变 / 上位效应 / 突变排序 benchmark 上**不可替代**；MGnify 把预算换成”给海量蛋白各画一个点”，赢在广度与绝对 ΔG，输在单蛋白深度——**这正是两者互补、最佳实践是合并而非替换的根本原因。**
...
**MGnify**：2,287,291 测定（1.86M 为 MGnify 来源）· 211,020 序列簇 · 129,303 结构簇 · 训练索引每域中位 **1** 条（均值 1.81，**67.9% 只有 WT**）· **替换 site-sat ≈0%**；**indel 深而专：仅 6,000 个域、各 ~49 条（共 295,607）** · 长度 60–80 aa · ΔG 均值 1.46。
