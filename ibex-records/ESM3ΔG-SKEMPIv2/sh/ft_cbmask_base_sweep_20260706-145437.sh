#!/bin/bash
#SBATCH --job-name=esm3dg_ftcb_base
...
# exp4 sweep — WEIGHTSET = BASE (ESM3dG_weights_1, MGnify-only). Construction = chainbreak + mask_pipe.
# array idx 0..4 -> lr; max-epochs 50; save every 10ep (ep10/20/30/40 + final ep50). No val.
LRS=(1e-6 5e-6 1e-5 5e-5 1e-4)
LR=${LRS[$SLURM_ARRAY_TASK_ID]}
echo "=== WEIGHTSET=base  array=$SLURM_ARRAY_TASK_ID  lr=$LR ==="
...
python esm3dg_stab/finetune.py \
  ...
  --lr $LR --seed 0 \
  --out cache/esm3dg_skempi_cbmask_base_lr${LR}.pt \
  --run_name ft_cbmask_base_lr${LR}
echo "DONE base lr=$LR"
