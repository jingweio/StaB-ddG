...
#SBATCH --output=/ibex/user/guoj0f/StaB-ddG/MGnify-replace/ibex-records/ESM3ΔG-SKEMPIv2/esm3dg_skempi_aug1_20260630-114905_%j.out
#SBATCH --error=/ibex/user/guoj0f/StaB-ddG/MGnify-replace/ibex-records/ESM3ΔG-SKEMPIv2/esm3dg_skempi_aug1_20260630-114905_%j.err

set -euo pipefail
source /ibex/user/guoj0f/anaconda3/etc/profile.d/conda.sh
conda activate esm-backbone   # esm 3.3.0 + torch 2.5.1 + peft + gemmi (esm3dg deps; clone OOM-killed on login node)
...
srun python -u eval.py --lora_ckpt $LORA --checkpoint ../cache/$RUN.pt --split test \
  ...
  --out ../ibex-records/ESM3ΔG-SKEMPIv2/results/eval_$RUN.csv
echo "===== DONE ====="
